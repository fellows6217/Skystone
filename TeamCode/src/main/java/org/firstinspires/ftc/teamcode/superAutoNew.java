...
abstract public class superAutoNew extends LinearOpMode {
    ...
    public float tocm(float meters) {
        float cm = meters * 100;
        return cm;
    }

    public float tomm(float meters) {
        float mm = meters * 1000;
        return mm;
    }

    void smartTranslate(double posx, double posy, float distx, float disty, double power) {
        double FRBLPower = ((-posy) - posx) * power;
        double FLBRPower = ((-posy) + posx) * power;
        float fudgeFactor = 1f;  //location accuracy +/- in m, cm, or mm
        int i = 0;
        float totalxAccel = 0;
        float avexAccel = 0;
        float currDistx = 0;
        ...
        while (opModeIsActive()) {
            ...
            totalxAccel = totalxAccel + getxAccel();
            totalyAccel = totalyAccel + getyAccel();
            ...
            if ( (xDiff > fudgeFactor ) || (yDiff > fudgeFactor)) {
    ...
    void smartGyroPivot(double target) {
        //Provide target as a gyro heading (-179 to 180)

        //Make sure it's a gyro heading (trap and convert if a compass heading is provided)
        target = compassToGyro(target);

        //First set up variables

        float fudgeFactor = .25f;  //heading accuracy +/- in degrees
        double wheelPower = .4;
        int turnDirection;
        ...
        while (opModeIsActive()) {
            ...
            double distance = target - currentHeading;
            if (distance < 0)
            ...
            if (((turnDirection = 1) && (180 > gyroToCompass(currentHeading)) && (180 < gyroToCompass(target))) || ((turnDirection = -1) && (180 < gyroToCompass(currentHeading)) && (180 > gyroToCompass(target)))) {
                currentHeading = gyroToCompass(currentHeading);
                target = gyroToCompass(target);
            }

            if ((turnDirection < 0) && (Math.abs(distance >= fudgeFactor)))
                pivotStates currentPivotState = pivotStates.Left;
            else if ((turnDirection > 0) && (Math.abs(distance >= fudgeFactor)))
                pivotStates currentPivotState = pivotStates.Right;
            else
                pivotStates currentPivotState = pivotStates.None;
    ...
    enum pivotStates {None, Left, Right}
